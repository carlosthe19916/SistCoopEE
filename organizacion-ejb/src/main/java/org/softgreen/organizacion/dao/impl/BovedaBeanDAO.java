 ...
 */
package org.softgreen.organizacion.dao.impl;
...
public class BovedaBeanDAO implements DAO<Integer, Boveda> {

	@PersistenceContext
	private EntityManager em;

	public Boveda create(Boveda t) {
		this.em.persist(t);
		return t;
	}

	public void delete(Boveda t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public Boveda find(Integer id) {
		return this.em.find(Boveda.class, id);
	}

	public Boveda update(Boveda t) {
	...
	public List<Boveda> findAll() {
		...
		cq.select(cq.from(Boveda.class));
		list = this.em.createQuery(cq).getResultList();
		return list;
	}

	public List<Boveda> findRange(int[] range) {
	...
	public int count() {
		javax.persistence.criteria.CriteriaQuery cq = this.em
				.getCriteriaBuilder().createQuery();
		javax.persistence.criteria.Root<Boveda> rt = cq.from(Boveda.class);
	...
	public List<Boveda> findByNamedQuery(String queryName, int resultLimit) {
	...
	public List<Boveda> findByNamedQuery(String namedQueryName,
			Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());

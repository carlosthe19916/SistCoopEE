...
public class AgenciaBeanDAO implements DAO<Integer, Agencia> {
	...
	public Agencia create(Agencia t) {
	...
	public void delete(Agencia t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public Agencia find(Integer id) {
		return this.em.find(Agencia.class, id);
	}

	public Agencia update(Agencia t) {
		Set<ConstraintViolation<Agencia>> violations = validator.validate(t);
		if (!violations.isEmpty()) {
			throw new ConstraintViolationException(
					new HashSet<ConstraintViolation<?>>(violations));
	...
	public List<Agencia> findRange(int[] range) {
	...
	public int count() {
		javax.persistence.criteria.CriteriaQuery cq = this.em
				.getCriteriaBuilder().createQuery();
		javax.persistence.criteria.Root<Agencia> rt = cq.from(Agencia.class);
	...
	public List<Agencia> findByNamedQuery(String queryName, int resultLimit) {

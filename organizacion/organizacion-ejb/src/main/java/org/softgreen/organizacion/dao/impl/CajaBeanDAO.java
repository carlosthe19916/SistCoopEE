...
public class CajaBeanDAO implements DAO<Integer, Caja> {

	@PersistenceContext
	private EntityManager em;

	public Caja create(Caja t) {
		this.em.persist(t);
		return t;
	}

	public void delete(Caja t) {
		t = this.em.merge(t);
		this.em.remove(t);
	}

	public Caja find(Integer id) {
		return this.em.find(Caja.class, id);
	}

	public Caja update(Caja t) {
    ...
    public int count() {
        javax.persistence.criteria.CriteriaQuery cq = this.em.getCriteriaBuilder().createQuery();
        javax.persistence.criteria.Root<Caja> rt = cq.from(Caja.class);
	...
	public List<Caja> findByNamedQuery(String queryName,
			int resultLimit) {
	...
	public List<Caja> findByNamedQuery(String namedQueryName, Map<String, Object> parameters, Integer offset, Integer limit) {
		Set<Entry<String, Object>> rawParameters = parameters.entrySet();
		Query query = this.em.createNamedQuery(namedQueryName);
		for (Entry<String, Object> entry : rawParameters) {
			query.setParameter(entry.getKey(), entry.getValue());
